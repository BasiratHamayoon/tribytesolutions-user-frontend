...
import { FiArrowRight, FiMapPin, FiClock, FiBriefcase, FiStar } from "react-icons/fi";
      ...
      {particles.map((p) => (
        <div
          ...
          style={{
            ...
            left: `${p.x}%`,
            top: `${p.y}%`,
            background: p.isOrange ? "rgba(255,107,0,0.25)" : "rgba(128,128,128,0.08)",
...
function SkeletonJobCard() {
  return (
    <div
      className="border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] p-5 animate-pulse"
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 space-y-2">
          ...
          <div className="h-4 w-40 bg-gray-200 dark:bg-white/[0.06] rounded" />
          <div className="flex gap-3">
            <div className="h-3.5 w-16 bg-gray-100 dark:bg-white/[0.04] rounded" />
            <div className="h-3.5 w-14 bg-gray-100 dark:bg-white/[0.04] rounded" />
            <div className="h-3.5 w-12 bg-gray-100 dark:bg-white/[0.04] rounded" />
          </div>
        </div>
        <div className="h-7 w-16 bg-gray-200 dark:bg-white/[0.06] rounded" />
      </div>
    </div>
  );
}
...
export default function HomeJobsSection() {
  const { active, loading, error } = useJobs();
  const [inView, setInView] = useState(false);
  const [hoveredIndex, setHoveredIndex] = useState(null);
  const ref = useRef(null);

  const displayJobs = active.slice(0, 6);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        />
        <div
          className="absolute -top-60 right-0 w-[500px] h-[500px] rounded-full opacity-[0.03]"
      ...
      <div className="relative z-10 max-w-[1280px] mx-auto px-6 sm:px-10 lg:px-16">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12">
            ...
            <div style={t(150)}>
              <h2 className="font-heading font-bold text-3xl sm:text-4xl lg:text-[2.8rem] text-gray-900 dark:text-white tracking-[-0.03em] leading-tight">
                ...
                <span className="relative inline-block text-orange-500">
                  world-class
                  <svg className="absolute -bottom-1.5 left-0 w-full" height="5" viewBox="0 0 200 5" fill="none">
                    <path
            ...
            <div style={t(300)}>
              <p className="mt-4 text-[14px] text-gray-500 dark:text-gray-400 leading-relaxed max-w-md">
                {error
                  ...
                  : "We're always looking for talented people passionate about building great products. Remote-first, competitive pay, and a culture you'll love."}
              </p>
            </div>
          </div>
          ...
          <div style={t(400)}>
            <Link
              href="/careers"
              className="group inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-[12.5px] font-bold transition-all duration-300 hover:-translate-y-[1px]"
              style={{ borderRadius: "4px" }}
            >
              View all openings
          ...
          {loading ? (
            ...
            <div className="lg:col-span-2 text-center py-12">
              <p className="text-[13px] text-gray-400 dark:text-gray-500">
                {error ? "Unable to load jobs." : "No open positions right now. Check back soon!"}
              </p>
            </div>
          ) : (
            displayJobs.map((job, i) => {
              ...
              return (
                <div
                  ...
                  className="group relative border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.02] hover:border-orange-500/30 dark:hover:border-orange-500/25 hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-all duration-400"
                  style={{ borderRadius: "4px" }}
                >
                  ...
                  <div className="flex items-center justify-between p-5 gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        {job.isFeatured && (
                          <span
                            className="px-1.5 py-0.5 text-[8px] font-bold bg-gray-100 dark:bg-white/[0.06] text-gray-500 dark:text-gray-300 uppercase tracking-wider flex items-center gap-1"
                            style={{ borderRadius: "3px" }}
                          >
                            <FiStar className="w-2 h-2 fill-current" /> Featured
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex-shrink-0">
                      <Link
                        ...
                        className="group/btn inline-flex items-center gap-1.5 px-4 py-1.5 border border-gray-200 dark:border-white/[0.1] text-gray-700 dark:text-white text-[11px] font-bold hover:border-orange-500 hover:text-orange-500 transition-all duration-300 hover:-translate-y-[1px]"
                        style={{ borderRadius: "4px" }}
                      >
                        Apply
                  ...
                  {hoveredIndex === i && (
                    <div
                      ...
                      style={{
                        background: "radial-gradient(ellipse at 50% 0%, rgba(255,107,0,0.04) 0%, transparent 70%)",
                        borderRadius: "4px",
                      }}
                    />
                  )}
