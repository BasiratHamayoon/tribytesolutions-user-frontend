...
export default function ServicesTech() {
  ...
  return (
    <section ref={ref} className="relative w-full bg-white dark:bg-[#09090b] py-16 overflow-hidden border-t border-gray-100 dark:border-white/[0.04]">
            ...
            return (
              <div key={i} className="flex-shrink-0 mx-2 group cursor-default">
                <div className="flex items-center gap-2.5 px-4 py-2 group-hover:scale-105 transition-all duration-300">
                  <Icon className={`w-5 h-5 ${!tech.color ? "text-gray-700 dark:text-white" : ""}`} style={tech.color ? { color: tech.color } : undefined} />
                  <span className="text-[12px] font-bold text-gray-700 dark:text-gray-300 group-hover:text-orange-500 transition-colors duration-300 whitespace-nowrap">{tech.name}</span>
                </div>
              </div>
            );
          })}
